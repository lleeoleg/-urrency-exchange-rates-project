...
from config import *
import add_rate_window
import edit_rate_window
import edit_time_window
...
class MainWindow:
    ...
    def create_toolbar(self):
        ...
        ttk.Button(toolbar, text="Обзор (Добавление с Excel)", 
                  command=self.load_from_excel).pack(side=tk.LEFT, padx=2)
        ttk.Button(toolbar, text="Добавить курс", 
                  command=self.add_rate).pack(side=tk.LEFT, padx=2)
        ttk.Button(toolbar, text="Санкционировать последние", 
                  command=self.sanction_latest).pack(side=tk.LEFT, padx=2)
        ttk.Button(toolbar, text="Удалить несанкционированные", 
                  command=self.delete_unsanctioned).pack(side=tk.LEFT, padx=2)
        ...
        ttk.Button(toolbar, text="Редактировать время", 
                  command=self.edit_time).pack(side=tk.LEFT, padx=2)
    ...
    def load_nb_rates(self):
        ...
        if result['success']:
            messagebox.showinfo("Успех", f"Загружено курсов: {result['loaded']}")
        else:
            messagebox.showerror("Ошибка", f"Ошибка загрузки: {result.get('error', '')}")
